...
class SingleSessionGuard {
  constructor(options={}){
    const baseDir = options.dataDir || process.env.ATOMIC_DATA_DIR || process.env.RENDER_DATA_DIR || process.cwd();
    this.lockPath = options.lockPath || process.env.ATOMIC_SESSION_LOCK_PATH || path.join(baseDir, 'session.lock');
    this.ttlMs = options.ttlMs || parseInt(process.env.ATOMIC_SESSION_TTL_MS || '900000', 10); // default 15m
    this.force = options.force || (process.env.ATOMIC_FORCE_LOCK === '1' || process.env.ATOMIC_FORCE_LOCK === 'true');
  ...
  acquire(){
    ...
    if (fs.existsSync(this.lockPath)){
      ...
      if (this.isStale(existing) || this.force){
        ...
        const msg = `Another ATOMIC FCA session appears active (lock: ${this.lockPath}). Set ATOMIC_FORCE_LOCK=true to override or wait until stale.`;
        const error = new Error(msg);
        error.code = 'ATOMIC_MULTIPLE_SESSIONS';
        throw error;
      }
    }
    const payload = {
