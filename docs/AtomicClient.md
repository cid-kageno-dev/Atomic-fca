# AtomicClient Documentation

The `AtomicClient` is a modern, event-driven client that provides a Discord.js-style API for easier bot development with ATOMIC FCA.
...
const options = {
    ...
    // Database
    databasePath: './atomic.db',      // Database file path
...
```typescript
import { AtomicClient, AtomicMessage, AtomicClientOptions } from 'atomic-fca';

const options: AtomicClientOptions = {
    prefix: '!',
    rateLimitEnabled: true,
    logLevel: 'info'
};

const client = new AtomicClient(options);

client.on('message', async (message: AtomicMessage) => {
    if (message.body === 'ping') {
        await message.reply('Pong!');
    }
});
...
const client = new AtomicClient({ prefix: '!' });
