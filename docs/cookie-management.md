...
This document explains how ATOMIC FCA handles cookie management to prevent rapid session expiry and improve overall stability.
...
ATOMIC FCA implements a robust multi-layered approach to maintain session stability:
...
- Configuration via `ATOMIC_SESSION_LOCK_PATH` and `ATOMIC_FORCE_LOCK`
...
3. **Set Region**: Use `ATOMIC_REGION` to maintain a consistent region
...
```
# Session stability
ATOMIC_PERSISTENT_DEVICE=true    # Keep device fingerprint consistent (default)
ATOMIC_DEVICE_FILE=./device.json # Custom device profile path
ATOMIC_SESSION_LOCK_PATH=./lock  # Single session lock file location
ATOMIC_REGION=NA                 # Fixed region (NA, EU, AS, etc.)

# Safety settings
ATOMIC_FCA_ULTRA_SAFE_MODE=1     # Maximum safety for stability
```
