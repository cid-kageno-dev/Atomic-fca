# Configuration Reference (ATOMIC FCA 3.0)
...
- `ATOMIC_DATA_DIR` – Base directory for persistent files (fallback: `RENDER_DATA_DIR` or CWD)
- `ATOMIC_APPSTATE_PATH` – Path to appstate.json
- `ATOMIC_CREDENTIALS_PATH` – Path to credentials.json (integrated login)
- `ATOMIC_BACKUP_PATH` – Directory for appstate backups
- `ATOMIC_DEVICE_FILE` – Path to persistent-device.json
- `ATOMIC_PERSISTENT_DEVICE` = true|false – Keep stable device fingerprint

### 3.2 Networking & headers
- `ATOMIC_PROXY` – HTTP(S) proxy URL (also respects `HTTPS_PROXY` / `HTTP_PROXY`)
- `ATOMIC_ACCEPT_LANGUAGE` – Example: `en-US,en;q=0.9`
- `ATOMIC_UA` – Override User-Agent
- `ATOMIC_REGION` – Force MQTT region (e.g., `HIL`)

### 3.3 Stability & preflight
- `ATOMIC_DISABLE_PREFLIGHT` = true|false – Skip heavy preflight validation
- `ATOMIC_ONLINE` = true|false – Override chat availability flag
- `ATOMIC_VERBOSE_MQTT` = true|false – Extra MQTT diagnostics logging

### 3.4 Single-session guard (prevents concurrent runs)
- `ATOMIC_SESSION_LOCK_PATH` – Path for session lock file (default: `<DATA_DIR>/session.lock`)
- `ATOMIC_SESSION_TTL_MS` – Lock stale timeout (default: 900000 = 15m)
- `ATOMIC_FORCE_LOCK` = true|false – Force takeover if lock present

### 3.5 Safety layer & allow/block lists
- `ATOMIC_FCA_ULTRA_SAFE_MODE` = '1' – Maximum protection presets
- `ATOMIC_FCA_SAFE_MODE` = '1' – Safe mode presets
- `ATOMIC_FCA_ALLOW_LIST` – Comma-separated userIDs allowed
- `ATOMIC_FCA_BLOCK_LIST` – Comma-separated userIDs blocked
...
- `DEBUG` – e.g. `atomic-fca:*` to enable debug output in certain examples

---
## 4) Integrated ATOMIC Login System options

Used internally for credential-based login; also exported for direct use via `IntegratedAtomicLoginSystem`. Options can be provided in code or via the env overrides above.

- Paths & persistence
  - `appstatePath` (env: `ATOMIC_APPSTATE_PATH`)
  - `credentialsPath` (env: `ATOMIC_CREDENTIALS_PATH`)
  - `backupPath` (env: `ATOMIC_BACKUP_PATH`)
  - `persistentDevice` (env: `ATOMIC_PERSISTENT_DEVICE`)
  - `persistentDeviceFile` (env: `ATOMIC_DEVICE_FILE`)
...
- Keepalives & diagnostics: `ATOMIC_VERBOSE_MQTT` for extra logs
- Headers: `acceptLanguage`, `userAgent`, `proxy`
- Region override: `ATOMIC_REGION`
...
- Temporarily increase MQTT verbosity with `ATOMIC_VERBOSE_MQTT=true` when debugging
...
|--------------------------------|------------|---------------|-----------------|-----|
| online                         | ✅         | ✅            |                 | ✅ (`ATOMIC_ONLINE`) |
| selfListen / listenEvents      | ✅         | ✅            |                 |     |
| updatePresence                 | ✅         | ✅            |                 |     |
| autoMarkDelivery / autoMarkRead| ✅         | ✅            |                 |     |
| autoReconnect                  | ✅         | ✅            |                 |     |
| userAgent                      | ✅         | ✅            | ✅               | ✅ (`ATOMIC_UA`) |
| proxy                          | ✅         | ✅            | ✅               | ✅ (`ATOMIC_PROXY`/`HTTPS_PROXY`/`HTTP_PROXY`) |
| acceptLanguage                 | ✅         | ✅            |                 | ✅ (`ATOMIC_ACCEPT_LANGUAGE`) |
| disablePreflight               | via `enableLazyPreflight(false)` | ✅ |       | ✅ (`ATOMIC_DISABLE_PREFLIGHT`) |
| pageID                         | ✅         | ✅            |                 |     |
| backoff/edit settings          | helper fns |               |                 |     |
| data/appstate/backup/device    |            | via Integrated Login |         | ✅ (`ATOMIC_*` paths) |
