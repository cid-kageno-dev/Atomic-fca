...
function fetchSeqID(defaultFuncs, api, ctx, callback, retryCount = 0) {
  ...
  const verboseMqtt = (ctx.globalOptions && ctx.globalOptions.verboseMqtt) || process.env.ATOMIC_VERBOSE_MQTT === '1';
...
function getBackoffState(ctx) {
  const envBase = parseInt(process.env.ATOMIC_MQTT_BACKOFF_BASE, 10) || 1000;
  const envMax = parseInt(process.env.ATOMIC_MQTT_BACKOFF_MAX, 10) || (5 * 60 * 1000);
  const envFactor = parseFloat(process.env.ATOMIC_MQTT_BACKOFF_FACTOR) || 2;
...
function shouldRunPreflight(ctx) {
  ...
  if (process.env.ATOMIC_DISABLE_PREFLIGHT === '1' || process.env.ATOMIC_DISABLE_PREFLIGHT === 'true') return false;
...
function buildStream(options, WebSocket, Proxy) {
  ...
  WebSocket.onopen = () => {
    ...
    const pingMs = parseInt(process.env.ATOMIC_MQTT_PING_INTERVAL, 10) || 9000;
...
function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
  ...
  if (verboseMqtt) {
    log.info('listenMqtt', `Starting ATOMIC MQTT bridge (attempt=${ctx._mqttDiag.attempts}, backoff=${backoff.current || 0}ms)`);
  }
  ...
  if (runPreflight) {
    (async () => {
      try {
        ...
        if (backoff.consecutiveFails >= 10 && global.__ATOMIC_COOKIE_REFRESHER__) {
          try {
            log.info("listenMqtt", "Triggering proactive cookie refresh after repeated session failures...");
            await global.__ATOMIC_COOKIE_REFRESHER__.performRefresh();
          } catch (refreshErr) {
  ...
  const options = {
    ...
    wsOptions: {
      headers: {
        ...
        "Accept-Language": (ctx.globalOptions && ctx.globalOptions.acceptLanguage) || process.env.ATOMIC_ACCEPT_LANGUAGE || "en-US,en;q=0.9",
  ...
  if (ctx.globalOptions.proxy === undefined) {
    const envProxy = process.env.ATOMIC_PROXY || process.env.HTTPS_PROXY || process.env.HTTP_PROXY;
  ...
  mqttClient.on("connect", function () {
    ...
    if (verboseMqtt) {
      log.info('listenMqtt', `ATOMIC MQTT bridge established in ${(Date.now() - attemptStartTs)}ms (attempt=${ctx._mqttDiag.attempts}).`);
    }
