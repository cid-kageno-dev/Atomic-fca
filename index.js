"use strict";
// ATOMIC FCA: Advanced and Safe Facebook Chat API (Enhanced Version)
...
if (process.env.PORT || process.env.ATOMIC_ENABLE_HEALTH_SERVER === '1') {
...
function buildAPI(globalOptions, html, jar) {
  ...
  // Professional gradient banner for ATOMIC FCA
  logger('🚀 Launching ATOMIC FCA Core...', 'info');
  ...
  mainPromise
    .then(async () => {
      ...
      try {
        ...
        const refreshEnabled = process.env.ATOMIC_COOKIE_REFRESH_ENABLED ? envBool(process.env.ATOMIC_COOKIE_REFRESH_ENABLED) : true;
        const refreshInterval = toInt(process.env.ATOMIC_COOKIE_REFRESH_INTERVAL, 30 * 60 * 1000);
        const expiryDays = toInt(process.env.ATOMIC_COOKIE_EXPIRY_DAYS, 90);
        const maxBackups = toInt(process.env.ATOMIC_COOKIE_MAX_BACKUPS, 5);
        const backupsEnabled = process.env.ATOMIC_COOKIE_BACKUP_ENABLED ? envBool(process.env.ATOMIC_COOKIE_BACKUP_ENABLED) : true;
        ...
        const appstatePath = globalOptions.appstatePath || process.env.ATOMIC_APPSTATE_PATH || (ctx.dataDir ? path.join(ctx.dataDir, 'appstate.json') : null);
        const backupPath = process.env.ATOMIC_COOKIE_BACKUP_PATH || globalOptions.backupPath || (ctx.dataDir ? path.join(ctx.dataDir, 'backups') : null);

        if (appstatePath) {
          ctx.cookieRefresher = cookieRefresher.initialize(ctx, utils, defaultFuncs, appstatePath, backupPath);
          global.__ATOMIC_COOKIE_REFRESHER__ = cookieRefresher; // Make accessible to MQTT for proactive refresh
...
// --- INTEGRATED ATOMIC LOGIN SYSTEM ---
// Full ATOMIC Login System integrated for npm package compatibility
const { TOTP } = require("totp-generator");

class IntegratedAtomicLoginSystem {
  constructor(options = {}) {
    const dataDir = process.env.ATOMIC_DATA_DIR || process.env.RENDER_DATA_DIR || process.cwd();
    const envPersistent = (v) => (v === '0' || v === 'false') ? false : (v === '1' || v === 'true') ? true : undefined;
    const envPD = envPersistent(process.env.ATOMIC_PERSISTENT_DEVICE);

    this.options = {
      appstatePath: options.appstatePath || process.env.ATOMIC_APPSTATE_PATH || path.join(dataDir, 'appstate.json'),
      credentialsPath: options.credentialsPath || process.env.ATOMIC_CREDENTIALS_PATH || path.join(dataDir, 'credentials.json'),
      backupPath: options.backupPath || process.env.ATOMIC_BACKUP_PATH || path.join(dataDir, 'backups'),
      ...
      persistentDeviceFile: options.persistentDeviceFile || process.env.ATOMIC_DEVICE_FILE || path.join(dataDir, 'persistent-device.json'),
      ...options
    };
...
// Integrated ATOMIC Login wrapper for easy usage
async function integratedAtomicLogin(credentials = null, options = {}) {
  printFancyStartupBanner();
  const loginSystem = new IntegratedAtomicLoginSystem(options);
  ...
  if (options.autoStartBot !== false && result.appstate) {
    // Phase 2: Start ATOMIC FCA bot with authenticated session
    ...
    try {
      ...
      const globalOptions = {
        ...
        emitReady: false,
        userAgent: process.env.ATOMIC_UA || "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36",
        proxy: process.env.ATOMIC_PROXY || process.env.HTTPS_PROXY || process.env.HTTP_PROXY,
        acceptLanguage: process.env.ATOMIC_ACCEPT_LANGUAGE || 'en-US,en;q=0.9',
        disablePreflight: process.env.ATOMIC_DISABLE_PREFLIGHT === '1' || process.env.ATOMIC_DISABLE_PREFLIGHT === 'true',
        appStatePath: result.appStatePath,
        ...options
      };

      return new Promise((resolve) => {
        // Initialize ATOMIC FCA bot with authenticated session
        ...
        loginHelper(result.appstate, null, null, globalOptions, (err, api) => {
          if (err) {
            ...
            Logger.success('READY', '🚀 ATOMIC FCA is now ready for use');
            ...
            resolve({
              ...
              appstate: result.appstate,
              method: result.method,
              message: 'ATOMIC FCA login successful'
            });
          }
        }, null);
      });
...
/**
 * Modern login entry point using Integrated ATOMIC Login System
...
async function login(loginData, options = {}, callback) {
  ...
  const mainLogger = {
    info: (message, details = null) => {
      console.log(`\x1b[36m[ATOMIC-FCA]\x1b[0m ${message}`);
      if (details && options.verbose) console.log(`\x1b[90m            → ${details}\x1b[0m`);
    },
    error: (message, details = null) => {
      console.log(`\x1b[31m[ATOMIC-FCA]\x1b[0m \x1b[31m${message}\x1b[0m`);
      if (details) console.log(`\x1b[90m            → ${details}\x1b[0m`);
    }
  };
  ...
  if (loginData.email || loginData.username || loginData.password) {
    ...
    try {
      ...
      const result = await integratedAtomicLogin({
      ...
      try {
        const envLockFlag = process.env.ATOMIC_SESSION_LOCK_ENABLED;
        ...
        if (lockEnabled) {
          const ssg = new SingleSessionGuard({ dataDir: process.env.ATOMIC_DATA_DIR });
          ssg.acquire();
          // keep guard reference to release on exit
          global.__ATOMIC_SSG__ = ssg;
        }
      } catch (e) {
...
module.exports.atomicLogin = integratedAtomicLogin; // Direct access to integrated login system
module.exports.IntegratedAtomicLoginSystem = IntegratedAtomicLoginSystem; // Class access
module.exports.setOptions = setOptions;
module.exports.utils = utils;
module.exports.logger = logger;
module.exports.FacebookSafety = FacebookSafety;
module.exports.AtomicClient = AtomicClient;
