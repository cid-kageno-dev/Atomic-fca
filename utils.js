...
function generateClientID() {
    ...
    const counter = (global.__ATOMIC_CLIENT_COUNTER__ = (global.__ATOMIC_CLIENT_COUNTER__ || 0) + 1);
    return `mqttwsclient_${timestamp}_${random}_${counter}`;
}

function getGUID() {
    // 1. Check Environment Variable (Best for Render/Heroku/Replit)
    if (process.env.ATOMIC_DEVICE_ID) {
        return process.env.ATOMIC_DEVICE_ID;
    }

    var devicePath = require("path").join(process.cwd(), "atomic_device.json");
    var fs = require("fs");
    ...
    log.warn("ATOMIC-FCA", "New Device ID generated: " + id);
    log.warn("ATOMIC-FCA", ">>> IF ON RENDER/HEROKU: Add 'ATOMIC_DEVICE_ID' = '" + id + "' to your Environment Variables to prevent bans! <<<");
...
function saveAppState(ctx) {
    ...
    try {
        ...
        if (process.env.ATOMIC_SYNC_ENV_FILE === '1') {
            fs.writeFileSync(envPath, JSON.stringify(appState));
        }

        // EXTERNAL PERSISTENCE: If ATOMIC_EXTERNAL_SAVE_URL is set, POST there
        if (process.env.ATOMIC_EXTERNAL_SAVE_URL) {
            const axios = require('axios');
            axios.post(process.env.ATOMIC_EXTERNAL_SAVE_URL, appState, {
                headers: { 'Content-Type': 'application/json', 'User-Agent': 'ATOMIC-FCA-Persistence' },
                timeout: 10000
            }).catch(err => {
...
// --- ATOMIC FCA Advanced Safety Utilities ---
 ...
 * Set process.env.ATOMIC_FCA_ULTRA_SAFE_MODE = '1' to enable maximum protection
 */
const ultraSafeMode = process.env.ATOMIC_FCA_ULTRA_SAFE_MODE === '1';
const safeMode = process.env.ATOMIC_FCA_SAFE_MODE === '1' || ultraSafeMode;

/**
 * Account protection lists for enhanced safety
 */
const allowList = process.env.ATOMIC_FCA_ALLOW_LIST ? process.env.ATOMIC_FCA_ALLOW_LIST.split(',') : null;
const blockList = process.env.ATOMIC_FCA_BLOCK_LIST ? process.env.ATOMIC_FCA_BLOCK_LIST.split(',') : null;
...
module.exports = {
    ...
    getAppStateB64: function () {
        if (process.env.ATOMIC_APPSTATE_B64) {
            try { return JSON.parse(Buffer.from(process.env.ATOMIC_APPSTATE_B64, 'base64').toString('utf8')); }
            catch (e) { log.error("utils", "Failed to parse ATOMIC_APPSTATE_B64"); return null; }
        }
        return null;
    },
    getType,
